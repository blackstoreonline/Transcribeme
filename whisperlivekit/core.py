    ...
    def _do_init(self, config=None, **kwargs):
        ...
        if config.diarization:
            if config.diarization_backend == "diart":
                ...
                raise ValueError(f"Unknown diarization backend: {config.diarization_backend}")
        ...
        if config.tts:
            try:
                ...
                self.tts_engine = TTSEngine(tts_voice=config.tts_voice)
                logger.info("TTS engine initialized with voice: %s", config.tts_voice)
